/* Convert a font file created by PXtoCH to a C header file.
Pixels are encoded serially across multiple bytes.
The metric table includes $x$ and $y$ offsets for drawing characters. */
...

{
	...
	fprintf(f, "struct { int index; char x, y, ncol, nrow; long w; } %s_metric[128] = {\n", s);
	...
	for (i = 0; i < 128; i++)
		...
		fprintf(f, "{%d, %d, %d, %d, %d, %ld},\n",
			...
			metric[i].internal_width,

			metric[i].data_row_count,
