...
The $dot$ function is used to multiply vectors and matrices.
...
The reason why $dot$ is necessary is that the software normally reorders factors
internally to optimize processing.
This is fine for scalar multiplication, but fails otherwise.
For example, $inv(A)*B$ in symbolic form is changed to $B*inv(A)$ internally.
Since the dot product is non-commutative, reordering gives the wrong result.
The $dot$ function avoids the problem because it does not reorder its arguments.
...
\medskip
\noindent
As the final example shows, an empty argument list indicates
that $d(f)$ is never zero.
